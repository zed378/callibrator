 ...
 * @param {object} entry
 * @param {{id: string, tenantId: string|null}} entry.user - the account changed
 * @param {string} [entry.actorId] - who did it (defaults to the user)
...
const auditCredentialChange = async (
  transaction,
  { user, actorId = user.id, operation, details = {}, ipAddress = null, userAgent = null },
) => {
  if (!user.tenantId) {
...
exports.verifyUserSession = async (userId, _session) => {
  ...
  return {
    ...
    data: {
      ...
      mfaRecoveryCodesRemaining: mfaService.recoveryCodesRemaining(user),
    },
  };
};
...
exports.loginMfa = async (userId, tokenCode, inputIp, inputUserAgent, { recoveryCode } = {}) => {
  ...
  const session = await openLoginSession({
    ...
    secondFactor: useRecovery
      ? async (transaction) => {
          if (!(await mfaService.consumeRecoveryCode(dbUser, recoveryCode, { transaction }))) {
            throw new AppError(401, "Invalid MFA code");
          }
          return { recoveryCodesRemaining: mfaService.recoveryCodesRemaining(dbUser) };
        }
      : undefined,
  });
  ...
  return {
    ...
    data: {
      ...
      mfaRecoveryCodesRemaining: mfaService.recoveryCodesRemaining(dbUser),
