...
It resolves **filesystem** paths, not storage **keys**. It is used by the legacy attachment path, by `backend/index.js` for the `.well-known` and `uploads/public` static mounts, by `getGlobalConfig()` for the `local` provider's default root, and by the migration tool to find the source file. It has no knowledge of tenants and performs no isolation check — the traversal guard in `storageMigration.legacyPath()` and in `attachment.service.resolveAbsPath()` is written at each call site. **Since S-15 (2026-09-24) both compare against one FIXED root, `path.resolve(storagePath("uploads"))`**, never against a root derived from the row's own `folder` (which is the value being validated); both separators are split, and an empty folder is refused. *(This paragraph said "re-derive the root" until S-34 caught it — that was the pre-S-15 behaviour, where the guard's root came from the untrusted input.)*

**Where uploads wait before they are scanned (S-17 deviation, recorded here per S-34).** S-17's card asked for quarantine *outside* the public tree. It is `uploads/.quarantine` — **inside** the uploads mount, as a dot-directory — because `/app/uploads` is its own bind mount/volume, and a quarantine beside it would be another filesystem: the promoting `rename` would fail with `EXDEV`, and the image does not make `/app` itself writable. It is never served: the `/uploads` static mount sets `dotfiles: "ignore"` (`upload.util.js#UPLOADS_STATIC_OPTIONS`, asserted by `tests/utils/upload.quarantine.s17.test.js`). Files a crash leaves there are removed hourly by the quarantine sweep (S-33, `QUARANTINE_SWEEP_SCHEDULER`, files older than `QUARANTINE_MAX_AGE_MINUTES`, default 60). Decision: [ADR-066](../../MEMORY/DECISIONS.md) (S-17 location).
