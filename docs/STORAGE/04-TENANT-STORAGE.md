...
**Where uploads wait before they are scanned (S-17 deviation, recorded here per S-34).** S-17's card asked for quarantine *outside* the public tree. It is `uploads/.quarantine` — **inside** the uploads mount, as a dot-directory — because `/app/uploads` is its own bind mount/volume, and a quarantine beside it would be another filesystem: the promoting `rename` would fail with `EXDEV`, and the image does not make `/app` itself writable. It is never served: the `/uploads` static mount sets `dotfiles: "ignore"` (`upload.util.js#UPLOADS_STATIC_OPTIONS`, asserted by `tests/utils/upload.quarantine.s17.test.js`). Files a crash leaves there are removed hourly by the quarantine sweep (S-33, `QUARANTINE_SWEEP_SCHEDULER`, files older than `QUARANTINE_MAX_AGE_MINUTES`, default 60). Decision: ADR-PENDING-infra (S-17 location).
